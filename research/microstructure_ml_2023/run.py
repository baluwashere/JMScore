...
USER_AGENT = "JMScore-research/0.2"
SECONDS = 86_400
...
ALL_DATES = TRAIN_DATES + CAL_DATES + HOLDOUT_DATES
...
def make_feature_row(state: dict[str, np.ndarray], t: int) -> tuple[list[float], int, float, float] | None:
    ...
    mid_entry=(bid[entry_s]+ask[entry_s])/2
    mid_exit=(bid[exit_s]+ask[exit_s])/2
    y=1 if mid_exit > mid_entry else 0
    long_gross=bid[exit_s]/ask[entry_s]-1.0
    short_gross=bid[entry_s]/ask[exit_s]-1.0
    return features, y, long_gross, short_gross


def build_dataset(days: tuple[date, ...]) -> tuple[Dataset, list[DayQA]]:
    rows=[]; ys=[]; timestamps=[]; lg=[]; sg=[]; qas=[]
    for day in days:
        ...
        for t in range(600, SECONDS-HORIZON_SEC-2, DECISION_STEP_SEC):
            ...
            if item is None:
                continue
            feat,y,long_gross,short_gross=item
            rows.append(feat); ys.append(y); timestamps.append(start_ms+t*1000); lg.append(long_gross); sg.append(short_gross)
        qa.candidate_snapshots=candidates; qa.usable_snapshots=sum(1 for ts in timestamps if start_ms <= ts < start_ms+86_400_000)
        qas.append(qa)
        print(json.dumps({"event":"day_done", **qa.__dict__}), flush=True)
    X=np.asarray(rows,dtype=np.float64); y=np.asarray(ys,dtype=np.int8); ts=np.asarray(timestamps,dtype=np.int64)
    long_gross=np.asarray(lg); short_gross=np.asarray(sg)
    return Dataset(X,y,ts,long_gross,short_gross,long_gross-TAKER_ROUNDTRIP,short_gross-TAKER_ROUNDTRIP), qas
...
def strategy_metric(ds: Dataset, p: np.ndarray, threshold: float) -> dict:
    gross=[]; net=[]; sides=[]; selected_ts=[]; next_eligible=-1
    for i,prob in enumerate(p):
        ts=int(ds.ts[i])
        if ts < next_eligible:
            continue
        side=1 if prob>=threshold else -1 if prob<=1-threshold else 0
        if side==0:
            continue
    ...
    return {
        "n":len(net), "gross_mean_bps":statistics.fmean(gross)*10_000,
        "net8_mean_bps":statistics.fmean(net)*10_000,
        "net8_win_rate":sum(v>0 for v in net)/len(net), "net8_pf":pf(net),
        "long_n":sum(s==1 for s in sides), "short_n":sum(s==-1 for s in sides),
        "selected_ts":selected_ts, "net_values":net,
    }
...
def model_metrics(y: np.ndarray, p: np.ndarray) -> dict:
    return {"roc_auc":float(roc_auc_score(y,p)),"brier":float(np.mean((p-y)**2)),"n":int(len(y)),"up_rate":float(np.mean(y))}


def choose_threshold(ds: Dataset, p: np.ndarray) -> tuple[float, list[dict]]:
    table=[]
    for threshold in THRESHOLDS:
        m=strategy_metric(ds,p,threshold)
        table.append({"threshold":threshold,**strip_internal(m)})
    eligible=[r for r in table if r["n"]>=50]
    if not eligible:
        return THRESHOLDS[0],table
...
def train_and_evaluate(name: str, model, train: Dataset, cal: Dataset, hold: Dataset) -> dict:
    model.fit(train.X,train.y)
    p_train=model.predict_proba(train.X)[:,1]
    p_cal=model.predict_proba(cal.X)[:,1]
    p_hold=model.predict_proba(hold.X)[:,1]
    threshold,table=choose_threshold(cal,p_cal)
    cal_strategy=strategy_metric(cal,p_cal,threshold)
    hold_strategy=strategy_metric(hold,p_hold,threshold)
    ci=daily_block_ci(hold_strategy)
    hold_public=strip_internal(hold_strategy); cal_public=strip_internal(cal_strategy)
    gate=(hold_public["n"]>=200 and hold_public["net8_mean_bps"] is not None and hold_public["net8_mean_bps"]>0 and (hold_public["net8_pf"] or 0)>1 and hold_public["gross_mean_bps"]>8 and cal_public["net8_mean_bps"] is not None and cal_public["net8_mean_bps"]>0)
    return {
        "model":name,"threshold":threshold,
        "predictive":{"train":model_metrics(train.y,p_train),"calibration":model_metrics(cal.y,p_cal),"holdout":model_metrics(hold.y,p_hold)},
        "calibration_threshold_table":table,
        "calibration_strategy":cal_public,
        "holdout_strategy":hold_public,
        "holdout_daily_block_bootstrap_95ci_net8_bps":ci,
        "passes_pilot_gate":gate,
    }


def dataset_summary(ds: Dataset) -> dict:
    return {"rows":int(len(ds.y)),"features":len(FEATURE_NAMES),"up_rate":float(np.mean(ds.y)) if len(ds.y) else None,"start_ts":int(ds.ts.min()) if len(ds.ts) else None,"end_ts":int(ds.ts.max()) if len(ds.ts) else None}
...
def main() -> None:
    ...
    if min(len(train.y),len(cal.y),len(hold.y))==0:
        raise RuntimeError("empty dataset split")

    logistic=Pipeline([("scale",StandardScaler()),("model",LogisticRegression(C=1.0,max_iter=500,random_state=SEED))])
    hgb=HistGradientBoostingClassifier(max_depth=3,learning_rate=0.05,max_iter=150,l2_regularization=1.0,random_state=SEED)
    results=[train_and_evaluate("logistic_regression",logistic,train,cal,hold),train_and_evaluate("hist_gradient_boosting",hgb,train,cal,hold)]

    result={
        "protocol":"research/microstructure_ml_2023/PROTOCOL.md",
        ...
        "models":results,
        "decision":"GO_FULL_2023" if any(r["passes_pilot_gate"] for r in results) else "NO_GO_PILOT",
        "versions":{"numpy":np.__version__},
    }
    import sklearn
    result["versions"]["scikit_learn"]=sklearn.__version__

    out=Path("research/microstructure_ml_2023/results"); out.mkdir(parents=True,exist_ok=True)
    (out/"pilot.json").write_text(json.dumps(result,indent=2),encoding="utf-8")

    lines=["# T4C — Microstructure + ML 2023 pilot","",f"Decision: **{result['decision']}**","",f"Train/calibration/holdout rows: {len(train.y)} / {len(cal.y)} / {len(hold.y)}","", "| Model | Threshold | Holdout N | Holdout AUC | Gross bps | Net8 bps | Net8 win % | Net8 PF | Bootstrap 95% CI net8 | Gate |","|---|---:|---:|---:|---:|---:|---:|---:|---|---|"]
    for r in results:
        ...
        lines.append(f"| {r['model']} | {r['threshold']:.2f} | {h['n']} | {r['predictive']['holdout']['roc_auc']:.4f} | {h['gross_mean_bps']:.3f} | {h['net8_mean_bps']:.3f} | {100*h['net8_win_rate']:.2f}% | {pfv} | {cis} | {'PASS' if r['passes_pilot_gate'] else 'FAIL'} |")
    lines += ["","## Calibration-selected thresholds",""]
    for r in results:
        c=r["calibration_strategy"]
        lines.append(f"- {r['model']}: threshold {r['threshold']:.2f}; N={c['n']}; gross={c['gross_mean_bps']:.3f} bps; net8={c['net8_mean_bps']:.3f} bps; PF={c['net8_pf']}")
    lines += ["","## Guardrails","","- 2024 and 2025 are not loaded.","- Spread is embedded using historical bid/ask for entry and exit.","- Primary net metric additionally subtracts 8 bps round-trip taker fees.","- Holdout is not used to select model thresholds.","- This sampled-day experiment is a pilot; a PASS requires full eligible-2023 replication before 2024 is opened.",""]
    text="\n".join(lines)
